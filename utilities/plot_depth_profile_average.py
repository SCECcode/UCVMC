...
##
#  @file plot_depth_profile_average.py
#  @brief Plots a depth profile or profiles using command-line parameters.
#  @If delta X and delta Y are defined, dx*dy points are used and multiple
#  @depth profile data are extracted and further post processed. 
#  @author 
#  @version 
#
#  Plots depth profile or profiles given a set of command-line parameters.

from pycvm import DepthProfileAverage, UCVM, VERSION, UCVM_CVMS, Point, ask_number, ask_path, ask_file, get_user_opts
import getopt, sys, os

## Prints usage statement.
def usage():
    print "Plots depth profile or profiles given a latitude, longitude, a depth,"
...
if ret_val == "bad":
    ...
    print ""
    print "Plot Depth-Profile/Profiles - UCVM %s" % VERSION
    print ""
...
###################################################################################
# Generate the average depth profile
d = DepthProfileAverage(Point(lon1, lat1, starting_depth), meta)
d.plot()
